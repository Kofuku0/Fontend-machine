...
const index = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await fetch(
        `https://dummyjson.com/products/search?q=${debouncedSearchTerm}&limit=10`
      );
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center ">
      ...
      <div className="p-2">Serach {debouncedSearchTerm} Results</div>
      <div>
        {data?.map((i, ix) => {
          return <div key={ix}>{i.title}</div>;
        })}
      </div>
    </div>
  );
